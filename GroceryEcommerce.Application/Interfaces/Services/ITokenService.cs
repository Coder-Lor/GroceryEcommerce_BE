...
public interface ITokenService
{
    ...
    Task<bool> CreateRefreshTokenInDatabaseAsync(Guid userId, string refreshToken, string? ipAddress = null);
    Task<RefreshToken?> GetRefreshTokenAsync(string token);
    Task<RefreshToken?> GetActiveRefreshTokenAsync(Guid userId);
    Task<bool> ValidateRefreshTokenAsync(string token);
    Task<bool> ValidateAndCreateRefreshTokenAsync(string token, Guid userId);
