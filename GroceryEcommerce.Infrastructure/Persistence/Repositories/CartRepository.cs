...
public class CartRepository(
    ...
    ILogger<AuthenticationRepository> logger): ICartRepository
{
    private readonly IDataAccessAdapterFactory _adapterFactory = adapterFactory;
    ...
    public async Task<Result<ShoppingCart?>> GetShoppingCartByUserIdAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            using var adapter = (DataAccessAdapter)_adapterFactory.CreateAdapter();
            var bucket = new RelationPredicateBucket(ShoppingCartFields.UserId == userId);
            var entity = await Task.Run(() => adapter.FetchNewEntity<ShoppingCartEntity>(bucket), cancellationToken);
    ...
    public async Task<Result<bool>> DeleteShoppingCartAsync(Guid cartId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var result = await adapter.DeleteEntityAsync(entity, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var entity = await Task.Run(() => adapter.FetchNewEntity<ShoppingCartItemEntity>(bucket), cancellationToken);
    ...
    public async Task<Result<ShoppingCartItem>> AddShoppingCartItemAsync(ShoppingCartItem item, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var saved =  await adapter.SaveEntityAsync(shoppingCartEntity, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var count = await adapter.DeleteEntitiesDirectlyAsync(typeof(ShoppingCartItemEntity), bucket, cancellationToken);
            if (count > 0)
            {
    ...
    public async Task<Result<Wishlist>> CreateWishlistAsync(Wishlist wishlist, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var saved =  await adapter.SaveEntityAsync(wishlistEntity, cancellationToken);
            if (saved)
            {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var entity = await Task.Run(() => adapter.FetchNewEntity<WishlistItemEntity>(bucket), cancellationToken);
            if (entity == null)
            {
    ...
    public async Task<Result<WishlistItem>> AddWishlistItemAsync(WishlistItem item, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var saved =  await adapter.SaveEntityAsync(wishlistItemEntity, cancellationToken);
            if (saved)
            {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var count = await adapter.DeleteEntitiesDirectlyAsync(typeof(WishlistItemEntity), bucket, cancellationToken);
            if (count > 0)
            {
    ...
    public async Task<Result<AbandonedCart>> CreateAbandonedCartAsync(AbandonedCart abandonedCart, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            using var adapter = (DataAccessAdapter)_adapterFactory.CreateAdapter();
            var saved = await adapter.SaveEntityAsync(abandonedCartEntity, cancellationToken);
            if (saved)
            {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var abandonedCartEntities = await adapter.FetchQueryAsync<AbandonedCartEntity>(
    ...
    public async Task<Result<int>> GetCartItemCountAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var cart = await Task.Run(() => adapter.FetchNewEntity<ShoppingCartEntity>(cartBucket), cancellationToken);
            ...
            var count = adapter.GetDbCount(new EntityCollection<ShoppingCartItemEntity>(), itemBucket);
...
public async Task<Result<bool>> ValidateCartItemsAsync(Guid cartId, CancellationToken cancellationToken = default)
{
    try
    {
        ...
        var itemBucket = new RelationPredicateBucket(ShoppingCartItemFields.CartId == cartId);
        var cartItems = new EntityCollection<ShoppingCartItemEntity>();

        var query = new QueryParameters()
        {
            CollectionToFetch = cartItems,
            FilterToUse = itemBucket.PredicateExpression
        };
        ...
        foreach (var item in cartItems)
        {
            ...
            var productBucket = new RelationPredicateBucket(ProductFields.ProductId == item.ProductId);
            var product = await Task.Run(() => adapter.FetchNewEntity<ProductEntity>(productBucket), cancellationToken);
